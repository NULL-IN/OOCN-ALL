"""
ping_gui.py – графічна утиліта пінгування v3
підтримує: одиночний ping + сканування мережі + збереження результатів у файли
...
def tab_scan(notebook, root):
    ...
    # зберігаємо посилання на результати для вкладки 3
...
# ══════════════════════════════════════════════════════════════════════════════
#  вкладка 3 — збереження результатів
# ══════════════════════════════════════════════════════════════════════════════
...
def tab_save(notebook, root, scan_results):
    ...
    active_entry = tk.Entry(files_frame, textvariable=active_path_var,
                            font=("Consolas", 10), bg=CLR["overlay"], fg=CLR["text"],
                            insertbackground=CLR["text"], relief="flat", width=36)
    active_entry.grid(row=0, column=1, ipady=4, padx=(0, 8))
    ...
    inactive_entry = tk.Entry(files_frame, textvariable=inactive_path_var,
                              font=("Consolas", 10), bg=CLR["overlay"], fg=CLR["text"],
                              insertbackground=CLR["text"], relief="flat", width=36)
    inactive_entry.grid(row=1, column=1, ipady=4, padx=(0, 8))
    tk.Button(files_frame, text="…", font=("Consolas", 10),
              bg=CLR["overlay"], fg=CLR["text"], relief="flat", cursor="hand2",
    ...
    pv_container = tk.Frame(preview_frame, bg=CLR["bg"])
    pv_container.pack(fill="both", expand=True)
    pv_container.columnconfigure(0, weight=1)
    pv_container.columnconfigure(1, weight=1)

    # активні
    tk.Label(pv_container, text="✔ Активні хости",
             font=("Consolas", 9, "bold"), fg=CLR["green"], bg=CLR["bg"]
             ).grid(row=0, column=0, sticky="w", padx=(0, 8))
    active_box = scrolledtext.ScrolledText(
        pv_container, font=("Consolas", 9), bg="#11111B", fg=CLR["green"],
    ...
    # неактивні
    tk.Label(pv_container, text="✘ Неактивні хости",
             font=("Consolas", 9, "bold"), fg=CLR["red"], bg=CLR["bg"]
             ).grid(row=0, column=1, sticky="w")
    inactive_box = scrolledtext.ScrolledText(
        pv_container, font=("Consolas", 9), bg="#11111B", fg=CLR["red"],
        relief="flat", width=30, height=14
    )
    inactive_box.grid(row=1, column=1, sticky="nsew")
    make_readonly(inactive_box)

    pv_container.rowconfigure(1, weight=1)
    ...
    def do_save():
        ...
        if not alive and not dead:
            messagebox.showwarning("Увага", "Немає даних для збереження.\nСпочатку виконайте сканування на вкладці «Сканування мережі».")
            return
    ...
    # кнопка оновлення попереднього перегляду
...
def build_gui():
    root = tk.Tk()
    root.title("Ping Utility v3 – Cisco Packet Tracer Style")
    ...
    hdr.pack(fill="x")
    tk.Label(hdr, text="Ping Utility v3",
             font=("Consolas", 14, "bold"), fg=CLR["text"], bg=CLR["surface"]).pack()
    tk.Label(hdr, text="Cisco Packet Tracer PC Command Line 1.0  |  ping + сканування + збереження результатів",

...
def scan_network(network_str, use_mask=False, mask_str=None):
    """
    ...
    print("-" * 35)
    print(f"\nРезультат: знайдено {len(alive)} активних хостів з {len(hosts)}")
    if alive:
        print("Активні хости:")
...
def main():
    ...
    print('Команди: ping <хост> [-n <к-сть>] | scan <мережа/cidr або діапазон> [-m <маска>] | exit\n')
    ...
    while True:
        ...
        # ────────── команда scan ──────────
            ...
            if len(parts) < 2:
                print("Використання: scan <мережа> [-m <маска>]")
                print("  Приклади:")
                print("    scan 192.168.1.0/24")
                print("    scan 192.168.1.1-192.168.1.50")
                print("    scan 192.168.1.0 -m 255.255.255.0")
                continue
            ...
            scan_network(network_str, use_mask, mask_str)
